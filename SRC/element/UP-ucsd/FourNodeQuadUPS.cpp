...
#include <cmath> //Rodrigo: added for sqrt, pow

void* OPS_FourNodeQuadUPS()
{
    if (OPS_GetNDM() != 2 || OPS_GetNDF() != 3) {
	opserr << "WARNING -- model dimensions and/or nodal DOF not compatible with QuadUP element\n";
	return 0;
    }
    if (OPS_GetNumRemainingInputArgs() < 12) { //Rodrigo: Increased to 12 from 11
	opserr << "WARNING insufficient arguments\n";
	opserr << "Want: element FourNodeQuadUPS eleTag? iNode? jNode? kNode? lNode? thk? type? matTag? bulk? rho? perm_x? perm_y? mVG? <b1? b2? pressure? dM? dK?>\n";
	return 0;
    }

    // FourNodeQuadUPSId, iNode, jNode, kNode, lNode
    int tags[5];
    int num = 5;
	...
	// bk, r, perm1, perm2, mVG   // Rodrigo: Added mVG
	double data[5];    //Rodrigo: increased to 5 from 4
	num = 5;        // Rodrigo: Increased to 5 from 4
    ...
    return new FourNodeQuadUPS(tags[0],tags[1],tags[2],tags[3],tags[4],
		*mat, "PlaneStrain", thk, data[0], data[1], data[2], data[3], data[4], // Rodrigo: Added data(4) for mVG 
			      opt[0],opt[1],opt[2]);
}

Matrix FourNodeQuadUPS::K(12,12);
Vector FourNodeQuadUPS::P(12);
double FourNodeQuadUPS::shp[3][4][4];
double FourNodeQuadUPS::pts[4][2];
double FourNodeQuadUPS::wts[4];
double FourNodeQuadUPS::dvol[4];
double FourNodeQuadUPS::shpBar[3][4];
Node *FourNodeQuadUPS::theNodes[4];


FourNodeQuadUPS::FourNodeQuadUPS(int tag, int nd1, int nd2, int nd3, int nd4,
			       NDMaterial &m, const char *type, double t, double bulk, 
			       double r, double p1, double p2,double mVG, double b1, double b2, double p)
:Element (tag, ELE_TAG_FourNodeQuadUPS),
  theMaterial(0), connectedExternalNodes(4),
  nd1Ptr(0), nd2Ptr(0), nd3Ptr(0), nd4Ptr(0), Ki(0),
	Q(12), pressureLoad(12), applyLoad(0), thickness(t), kc(bulk), rho(r), pressure(p), mVG(mVG),//Rodrigo: added mVG
 end1InitDisp(0),end2InitDisp(0),end3InitDisp(0),end4InitDisp(0)
{
    // Ensure m_vg is valid (<1.0 to compute n_vg)
    if (mVG >= 1.0) {
        opserr << "FourNodeQuadUPS::FourNodeQuadUPS - m_vg should be < 1 (got " << mVG << "), setting to 0.99\n";
        mVG = 0.99;
    }
    
	// Set the shape functions and quadrature points
  ...
  perm[1] = p2;
    ...
    if (theMaterial == 0) {
      opserr << "FourNodeQuadUPS::FourNodeQuadUPS - failed allocate material model pointer\n";
      exit(-1);
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      // Check allocation
      if (theMaterial[i] == 0) {
	opserr << "FourNodeQuadUPS::FourNodeQuadUPS -- failed to get a copy of material model\n";
	exit(-1);
      }
    }
...
FourNodeQuadUPS::FourNodeQuadUPS()
:Element (0,ELE_TAG_FourNodeQuadUP),
  theMaterial(0), connectedExternalNodes(4),
 nd1Ptr(0), nd2Ptr(0), nd3Ptr(0), nd4Ptr(0), Ki(0),
	Q(12), pressureLoad(12), applyLoad(0), thickness(0.0), kc(0.0), rho(0.0), pressure(0.0), mVG(0.0), //Rodrigo: Added mVG=initilization
 end1InitDisp(0),end2InitDisp(0),end3InitDisp(0),end4InitDisp(0)
{
...
int
FourNodeQuadUPS::getNumExternalNodes() const
{
    return 4;
}

const ID&
FourNodeQuadUPS::getExternalNodes()
{
    return connectedExternalNodes;
}

Node **
FourNodeQuadUPS::getNodePtrs()
{
  theNodes[0] = nd1Ptr;
  theNodes[1] = nd2Ptr;
  theNodes[2] = nd3Ptr;
...
int
FourNodeQuadUPS::getNumDOF()
{
    return 12;
}

void
FourNodeQuadUPS::setDomain(Domain *theDomain)
{
  ...
  if (nd1Ptr == 0 || nd2Ptr == 0 || nd3Ptr == 0 || nd4Ptr == 0) {
    //opserr << "FATAL ERROR FourNodeQuadUPS (tag: %d), node not found in domain",
    //	this->getTag());
  ...
  if (dofNd1 != 3 || dofNd2 != 3 || dofNd3 != 3 || dofNd4 != 3) {
    //opserr << "FATAL ERROR FourNodeQuadUPS (tag: %d), has differing number of DOFs at its nodes",
    //	this->getTag());
  ...
  // --- Definition of Changes of Saturation ---
  // Initializaze the vector of flags if the material supports getSr(), getdSdP()
  checkSaturation.resize(4);
  double Sr;
  double dSdP;  
  for (int i = 0; i < 4; i++) {
      theMaterial[i]->getSr(Sr);   // Actual Saturation (1 si no variable por defecto)
      theMaterial[i]->getdSdP(dSdP); // Derivative
      // If Sr deviates from 1.0 or dSdP is different from 0.0, we assume unsaturated behavior
      if (std::fabs(Sr - 1.0) > 1e-8 || std::fabs(dSdP) > 1e-12)
          checkSaturation[i] = true;
      else
          checkSaturation[i] = false;
  }
}

int
FourNodeQuadUPS::commitState() {
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "FourNodeQuad_UPS::commitState () - failed in base class";
    }
...
int
FourNodeQuadUPS::revertToLastCommit()
{
    int retVal = 0;
...
const Matrix&
FourNodeQuadUPS::getTangentStiff()
{
...
const Matrix &FourNodeQuadUPS::getInitialStiff ()
{
  if (Ki != 0) return *Ki;
  ...
  Ki = new Matrix(K);
  if (Ki == 0) {
    opserr << "FATAL FourNodeQuadUPS::getInitialStiff() -";
...
FourNodeQuadUPS::getDamp()
{
  ...
  for (i = 0; i < 12; i += 3) {
    ...
    for (j = 2; j < 12; j += 3) {
      ...
      for (m = 0; m < 4; m++) {
         double Se;
         if (checkSaturation[m]) {                               //Rodrigo: for checking degree of saturation
             theMaterial[m]->getSr(Sr);
			 theMaterial[m]->getSresidual(Sres); 
             Se = (Sr - Sres) / (1.0 - Sres);
             if (Se < 0.0) Se = 0.0;
             if (Se > 1.0) Se = 1.0;
         }
	    ...
	    Kdamp(i,j) += -dvol[m]*Se*shp[0][i1][m]*shp[2][j1][m];
	    Kdamp(i+1,j) += -dvol[m]*Se*shp[1][i1][m]*shp[2][j1][m];//Rodrigo: added due to equation of Bishop (chi_w) parameter
	  }
      Kdamp(j,i) = Kdamp(i,j);
  ...
  for (i = 2; i < 12; i += 3) {
    ...
    for (j = 2; j < 12; j += 3) {
      ...
      for (m = 0; m < 4; m++) {
          double kr = checkSaturation[m]
              ? getPermeability(theMaterial[m])
                  : 1.0;
	    Kdamp(i,j) += - dvol[m]*(kr*perm[0]*shp[0][i1][m]*shp[0][j1][m] +   //Rodrigo: added kr
	                  kr*perm[1]*shp[1][i1][m]*shp[1][j1][m]);
	  }
    }
  }
...
// Calculate kr (relative permeability) vía van Genuchten
double FourNodeQuadUPS::getPermeability(NDMaterial* mat)
{
    double Sr;
	mat->getSr(Sr); 
    double Sres;
    mat->getSresidual(Sres);
    double Se = (Sr - Sres) / (1.0 - Sres);
    if (Se < 0.0) Se = 0.0;
    if (Se > 1.0) Se = 1.0;
	// Calculate relative permeability using van Genuchten model    
    double kr = 0.0;
    if (Se > 0.0) {
        kr = sqrt(Se) * pow(1.0 - pow(1.0 - pow(Se, 1.0 / mVG), mVG), 2.0);
    }
    return kr;
}
double FourNodeQuadUPS::getPorosity(NDMaterial* mat) //Rodrigo: get porosity
{
    //double e = mat->getVoidRatio();
    double e;
    mat->getVoidRatio(e);
    return e / (1.0 + e);
}
const Matrix&
FourNodeQuadUPS::getMass()
{
  K.Zero();
  ...
  for (i = 2; i < 12; i += 3) {
    ...
    for (j = 2; j < 12; j += 3) {
      ...
      if (checkSaturation[m]) {
          theMaterial[m]->getPorosity(n);
          theMaterial[m]->getSr(Sr);
          theMaterial[m]->getdSdP(dSdp);
          double Cs = n * dSdp;
		  const double Kf = 2.2e9; // Rodrigo: Kf is the bulk modulus of the fluid      
		  const double alpha = 1.0;     // Rodrigo: alpha is the Biot coefficient 
          double invQ = Cs + n * Sr / Kf + (alpha - n) * Sr / kc;
          K(i, j) += -dvol[m] * invQ * shp[2][i1][m] * shp[2][j1][m];
      }
      else {
          // Si no hay saturación variable, comportamiento saturado puro (1/Kc)
...
int
FourNodeQuadUPS::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	...
	if (type == LOAD_TAG_SelfWeight) {
		...
		opserr << "FourNodeQuadUPS::addLoad - load type unknown for ele with tag: " << this->getTag() << endln;
		return -1;
	} 
...
int
FourNodeQuadUPS::addInertiaLoadToUnbalance(const Vector &accel)
{
  ...
  if (3 != Raccel1.Size() || 3 != Raccel2.Size() || 3 != Raccel3.Size() ||
      3 != Raccel4.Size()) {
    opserr << "FourNodeQuadUPS::addInertiaLoadToUnbalance matrix and vector sizes are incompatible\n";
    return -1;
  }
...
const Vector&
FourNodeQuadUPS::getResistingForce()
{
  P.Zero();
  ...
  for (int alpha = 0, ia = 0; alpha < 4; alpha++, ia += 3) {
    ...
    for (i = 0; i < 4; i++) {
		double kr = getPermeability(theMaterial[i]);
		if (applyLoad == 0) {
      		P(ia+2) += dvol[i]*rho*(kr*perm[0]*b[0]*shp[0][alpha][i] +
      		           kr*perm[1]*b[1]*shp[1][alpha][i]);
		} else {
			P(ia+2) += dvol[i]*rho*(kr*perm[0]*appliedB[0]*shp[0][alpha][i] +
      		           kr*perm[1]*appliedB[1]*shp[1][alpha][i]);
		}
    }
  }
...
const Vector&
FourNodeQuadUPS::getResistingForceIncInertia()
{
  int i, j, k;
...
int
FourNodeQuadUPS::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  static Vector data(14); //Rodrigo: changed size to 14 from 13 to include mVG
  data(0) = this->getTag();
  data(1) = thickness;
  data(2) = rho;
  data(3) = b[0];
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadUPS::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadUPS::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "WARNING FourNodeQuadUPS::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }
...
int
FourNodeQuadUPS::recvSelf(int commitTag, Channel &theChannel,
						FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadUPS::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadUPS::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new NDMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FourNodeQuadUPS::recvSelf() - Could not allocate NDMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	opserr << "FourNodeQuadUPS::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	return -1;
      }
      ...
      if (res < 0) {
opserr << "FourNodeQuadUPS::recvSelf() - material " << i << "failed to recv itself\n";
	return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
	...
	theMaterial[i] = theBroker.getNewNDMaterial(matClassTag);
	if (theMaterial[i] == 0) {
opserr << "FourNodeQuadUPS::recvSelf() - material " << i << "failed to create\n";
...
void
FourNodeQuadUPS::Print(OPS_Stream &s, int flag)
{
...
int
FourNodeQuadUPS::displaySelf(Renderer &theViewer, int displayMode, float fact, const char **argv, int numModes)
{
...
Response*
FourNodeQuadUPS::setResponse(const char **argv, int argc, OPS_Stream &output)
{
...
int
FourNodeQuadUPS::getResponse(int responseID, Information &eleInfo)
{
  switch (responseID) {
...
int
FourNodeQuadUPS::setParameter(const char **argv, int argc, Parameter &param)
{
  if (argc < 1)
    return -1;
  ...
  else if (strcmp(argv[0], "mVG") == 0) { //Rodrigo: van Genuchten parameter now we can modify in
...
int
FourNodeQuadUPS::updateParameter(int parameterID, Information &info)
{
  int res = -1;
  int matRes = res;
  switch (parameterID) {
...
void FourNodeQuadUPS::shapeFunction(void)
{
...
double FourNodeQuadUPS::mixtureRho(int i)
{
    double rho_s = theMaterial[i]->getRho(); // Density of the soil
    double rho_w = 1000.0;                  // Density of the water(kg/m^3)

    // Determine Sr en each point of the integration
    double Sr = 1.0;
	if (checkSaturation.size() == 4 && checkSaturation[i]) {//Rodrigo: check if saturation is defined
        theMaterial[i]->getSr(Sr);
    }
    // Porosity (n = e/(1+e))
    double n = this->getPorosity(theMaterial[i]);

    // Densidad de mezcla: (1-n)*rho_s + n*Sr*rho_w
    double rhoi = (1.0 - n) * rho_s + n * Sr * rho_w;
    return rhoi;
}

void FourNodeQuadUPS::setPressureLoadAtNodes(void)
{
        pressureLoad.Zero();
