...
#ifndef FourNodeQuadUPS_h
#define FourNodeQuadUPS_h
...
class FourNodeQuadUPS : public Element
{
  public:
    FourNodeQuadUPS(int tag, int nd1, int nd2, int nd3, int nd4,
		  NDMaterial &m, const char *type,
		  double t, double bulk, double rhof, double perm1, double perm2, double mVG,
		   double b1 = 0.0, double b2 = 0.0, double p = 0.0);

    FourNodeQuadUPS();
    virtual ~FourNodeQuadUPS();
    const char *getClassType(void) const {return "FourNodeQuadUPS";};
  ...
  private:
	...
	double mVG; //Van Genuchten parameter 
	...
	std::vector<bool> checkSaturation; //Rodrigo: added to check if the material has variable saturation
    ...
    // private member functions - only objects of this class can call these
    double getPermeability(NDMaterial* mat); // calculate kr según Van Genuchten
    double getPorosity(NDMaterial* mat);          // calculate void ratio
